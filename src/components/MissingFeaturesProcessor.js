...
function MissingFeaturesProcessor() {
  ...
  const filterObjectByConfig = (data, conf, path = "") => {
    return Object.keys(data).reduce((acc, key) => {
      ...
      if (configValue === false && key !== "total_tally") {
  ...
  const downloadFilteredData = () => {
    ...
    saveAs(blob, "missing-features.json");
  };

  return (
    <div>
      <button onClick={filterData}>Filter & Prepare Data</button>
      <button
...
export default MissingFeaturesProcessor;
